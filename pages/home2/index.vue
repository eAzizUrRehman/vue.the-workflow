...
export default {
  ...
  methods: {
      ...
      elements.forEach((element) => {
        ...
        if (element.children.length > 0) {
          element.children.forEach((child, index) => {
            const { childX, childY } = this.calculateChildPosition(
              element,
              index
            )
    ...
    drawText(ctx, element, x, y) {
      ...
      ctx.fillText(element.level, x + 50, y)
    },
    ...
    addChild(parentElement, type) {
      ...
      const hasThisChildAlready = parentElement.children.some((child) => {
        return child.type === type
      })

      if (hasThisChildAlready) return
      console.log('hasThisChildAlready ----', hasThisChildAlready)
      ...
      this.recalculateParentWidth(parentElement)
      this.setCanvas()
    },
    recalculateParentWidth(parentElement) {
      let width = parentElement.children.reduce((acc, child) => {
        return acc + child.width
      }, 0)

      parentElement.width += width
    },
    calculateChildPosition(parentElement, index) {
      const offsetX = 300 // Horizontal offset for each child
      const offsetY = 100 // Vertical offset for each child
      const childX = parentElement.x + (index % 2 === 0 ? -offsetX : offsetX)
      const childY = parentElement.y + offsetY
